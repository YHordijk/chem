...
class Display:
	...
	def draw_bond(self, surf, a1, a2, order, width=125, outline_width=3, draw_background=False):
		...
		if order == 1:
			if draw_background:
				rects.append(draw_line(surf, self.bkgr_colour, p1, p2, width + outline_width))
			else:
			...
			if not np.array_equal(p1,p2):
				...
				if draw_background:
					rects.append(draw_line(surf, self.bkgr_colour, p1-perp, p2-perp, width + outline_width))
				else:
	...
	def draw_atom(self, surf, a, size=300, outline_width=2):
		...
		rect = pg.draw.circle(surf, self.bkgr_colour, p, rad+outline_width)
	...
	def draw_mesh(self, params, mesh, fill=True):
		...
		# 		r = pg.draw.lines(surf, c, True, (vert_p[t[0]], vert_p[t[1]], vert_p[t[2]]))
		# 		if i == 0:
		# 			rect = r
		# 		rect = rect.union(r)
		# else:
		# 	points = []
		# 	for t in tria:
		# 		points += [vert_p[t[0]], vert_p[t[1]], vert_p[t[2]]]
		# 	rect = pg.draw.lines(surf, (255,255,255), False, points)

		# print('Draw time (s):', pc()-start)
		start = pc()
		params['rects'].append(rect)
